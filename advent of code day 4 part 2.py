...
# turn the string into a list so items can be reassigned
for line in inputfile:
    inputfile[inputfile.index(line)] = list(line)
...
while moveablecount != previouscount:
    previouscount = moveablecount
    for r in range(inputlength):
        for c in range(linelength):
            if inputfile[r][c] == "@":
                ...
                if canmove == True:
                    moveablecount += 1
                    # part 2 just set the coord to a . when confirmed it can be moved
                    # then you just repeat the algorithm until the count doesnt increase over a loop.
                    inputfile[r][c] = "."
